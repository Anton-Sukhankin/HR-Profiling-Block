...
- segmented control `Генерация | Анализ | Чат`;
- основную область вкладки;
- футер с полем ввода, если вкладка предполагает ввод.
...
- закрыть AI-панель;
- вернуть активную вкладку к `Генерация`;
- очистить временные результаты генерации и анализа;
- остановить отложенные операции генерации/анализа, чтобы старый результат не применился к новой форме.
...
Три режима должны быть визуально едины, но функционально различимы:

- `Генерация` — сценарии действия и поле `Опишите профиль`;
...
При входе во вкладку пользователь видит сообщение AI и набор быстрых сценариев.

Клик по быстрому сценарию вставляет текст в поле ввода. Генерация запускается только после отправки.

После отправки:

- запрос пользователя отображается как исходящее сообщение;
- появляется loader AI;
- после завершения появляется ответ AI о сформированном профиле;
- рабочая область заполняется демонстрационными значениями.

Поле ввода имеет фиксированную высоту. Кнопка отправки круглая, с иконкой, без текстовой подписи.
...
Чат визуально близок к генерации, но должен восприниматься как консультация.

Под приветственным сообщением отображаются быстрые вопросы. Поле ввода сохраняет стилистику генерации, но placeholder отличается: `Задайте вопрос`.
