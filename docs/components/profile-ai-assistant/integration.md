...
Действия анализа должны стремиться к выполнению через API контекста приложения, а не через изолированную логику панели.

Текущая реализация смешанная:

- действия второго этапа `Ключевые компетенции` выполняются через `profileCreateStageContext.applyCompetencyRecommendationAction`;
- часть действий первого этапа `Общие положения и функционал` пока выполняется через DOM-сценарии в `events/profile-ai-assistant.events.js`: клик по кнопке добавления сущности, установка значения в trigger, добавление функций и последующий вызов `refreshFunctionalStage`.

Это допустимо для текущего прототипа, но при дальнейшем развитии first-stage actions лучше постепенно переводить на явные методы контекста приложения.
...
AI-помощник инициирует действие. Для новых сценариев фактическое изменение данных должно проходить через согласованные функции рабочей области, чтобы избежать расхождения между DOM, моделью профиля и документацией.
